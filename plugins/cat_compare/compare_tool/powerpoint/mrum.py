...
def generate_powerpoint_from_mrum(
    ...
    """
    Legacy mrum generator, adapted to:
    ...
    cfg = config or {}
    log.debug("[mrum] Generating PowerPoint presentation...")
    ...
    try:
        ...
        if not os.path.exists(template_path):
            env_path = os.getenv("TEMPLATE_MRUM_PATH", "")
            if env_path and os.path.exists(env_path):
                template_path = env_path
            else:
                log.warning(
                    "[mrum] template_mrum.pptx not found at %s; "
            ...
            log.debug("[mrum] Template loaded from: %s", template_path)
        ...
        log.info("[mrum] Number of applications in the current 'Analysis' sheet: %s", number_of_apps)
        ...
        log.debug("[mrum] Loaded Summary sheet successfully.")
        log.debug("[mrum] Summary DataFrame head:\n%s", summary_df.head())
        ...
        def insert_table_at_placeholder(slide, placeholder_name, rows, cols):
            ...
            if not placeholder:
                log.error("[mrum] Placeholder '%s' not found on the slide.", placeholder_name)
                return None
        ...
        # ------------------------------------------------------------------
        try:
            curr_overall_df = pd.read_excel(current_file_path, sheet_name="OverallAssessmentMRUM")
        except Exception:
            curr_overall_df = pd.DataFrame()

        try:
            prev_overall_df = pd.read_excel(previous_file_path, sheet_name="OverallAssessmentMRUM")
        except Exception:
        ...
        oa_up, oa_down = count_changes(df_analysis, "OverallAssessmentMRUM")
        ...
        if key_callouts_slide:
            ...
            log.error("[mrum] No slide available to place Key Callouts table.")

        # ------------------------------------------------------------------
        # mrum maturity badge & notes (uses current Analysis)
        # ------------------------------------------------------------------
        try:
            ...
            log.warning("[mrum] Maturity badge/notes skipped: %s", e)
        ...
        if improved_slide:
            ...
            log.warning("[mrum] No slide found for Improved Applications.")
        ...
        if summary_slide:
            ...
            log.error("[mrum] No suitable slide found for Summary tables.")
        ...
        if overall_slide:
            ...
            log.warning("[mrum] Could not find slide for Overall Assessment.")

                # ============================
        # mrum Entity Comparison (Slide 8) — Table Placeholder 1
        # Headers: Metric | # of Apps Improved | # Apps Degraded | Overall Result | Percentage Value
        # Rows: NetworkRequestsmrum, HealthRulesAndAlertingmrum
        # ============================
        ...
        entity_slide = choose_slide_for_section(
            prefer_titles=[
                ...
                "Entity Comparison",
                "Comparison Result",
                "mrum Entity Comparison",
                "mrum Maturity Assessment Result",
            ],
            required_placeholders=("Table Placeholder 1",),
            exclude_ids=used_slide_ids,
        )
        ...
        if entity_slide and find_table_placeholder_by_name(entity_slide, "Table Placeholder 1"):
            ...
            # Compute results for each mrum area using the already-computed counters
            ...
            rows = [
                ["Network Requests (mrum)", str(net_up), str(net_down), net_result, f"{net_pct}%"],
                ["Health Rules & Alerting (mrum)", str(hra_up), str(hra_down), hra_result, f"{hra_pct}%"],
            ]
            ...
            logging.warning("[mrum] Entity comparison slide not found or missing 'Table Placeholder 1'.")

        # ============================
        # Slide 11: Network Requests Deep Dive (mrum)
        ...
        if deep_dive_slide:
            ...
            logging.debug("[mrum][Slide11] df_network_requests columns: %s", list(df_network_requests.columns))
            ...
            logging.debug("[mrum][Slide11] Rectangles: resolved app_col=%s", app_col_nr_eff)
            ...
            logging.debug(
                "[mrum][Slide11] Rectangles: metric columns -> collecting=%s, limitNotHit=%s, customRules=%s, btCorr=%s, cesInclude=%s",
                col_collecting,
                col_limit_nothit,
                col_custom_rules,
                col_bt_corr,
            ...
            def count_metric_declines(df, app_col, metric_col, is_bool, label):
                if not app_col or not metric_col:
                    logging.warning(
                        "[mrum][Slide11] Metric '%s' missing column (%s) or app_col (%s).",
                        label,
                        metric_col,
                        app_col,
                    )
                ...
                for _, r in df.iterrows():
                    ...
                    if (is_bool and is_bool_decline_cell(val)) or (
                        ...
                        apps.append(app)
                logging.info(
                    "[mrum][Slide11] Rectangles: %s declines=%d (apps sample: %s)",
                    label,
                    len(apps),
                    apps[:10],
                )
            ...
            if app_col_nr_eff and col_limit_nothit:
                ...
                if not concerto_rows.empty:
                    logging.debug(
                        "[mrum][Slide11] Concerto networkRequestLimitNotHit cell(s): %s",
        ...
        if deep_dive_slide and find_table_placeholder_by_name(
            ...
            logging.debug(
                "[mrum][Slide11] df_network_requests columns: %s",
                list(df_network_requests.columns),
            )
            logging.debug(
                "[mrum][Slide11] df_analysis columns: %s", list(df_analysis.columns)
            )
            ...
            grade_col_nr = first_present_col(
                ...
                [
                    ...
                    "networkRequestsGrade",
                    "MRUMNetworkRequestsGrade",   # ← added (capital MRUM)
                    "mrumNetworkRequestsGrade",   # keep lowercase variant just in case
            ...
            logging.debug(
                "[mrum][Slide11] Resolved app_col_nr=%s, grade_col_nr=%s",
                app_col_nr,
                grade_col_nr,
            )
            ...
            if not any(v > 0 for v in totals_by_grade.values()) and "NetworkRequestsMRUM" in df_analysis.columns:
                inferred_totals = {g: 0 for g in table_grades}
                grade_by_app = {}  # ← NEW: per-app map from Analysis
                for _, r in df_analysis.iterrows():
                    app = str(r.get("name", "") or "").strip()
                    _, cg = parse_transition(r.get("NetworkRequestsMRUM", ""))
                    disp = cg.capitalize() if cg else None
                    if app and disp in inferred_totals:
                        inferred_totals[disp] += 1
                        grade_by_app[app] = disp
                if any(inferred_totals.values()):
                    totals_by_grade = inferred_totals
                    logging.warning(
                        "[mrum][Slide11] Using inferred per-grade totals from Analysis: %s",
                        totals_by_grade,
                    )
            ...
            logging.info(
                "[mrum][Slide11] Declined apps from Analysis: %d",
                len(declined_apps_analysis),
            )
            logging.debug(
                "[mrum][Slide11] Declined apps (Analysis) sample: %s",
            ...
            logging.info(
                "[mrum][Slide11] Declined apps from NR metrics: %d",
                len(metric_declined_apps),
            )
            logging.debug(
                "[mrum][Slide11] Declined apps (Metrics) sample: %s",
            ...
            logging.info(
                "[mrum][Slide11] Total declined apps (union): %d", len(declined_union)
            )
            ...
            for app in sorted(declined_union):
                ...
                # Fallback: if no grade can be resolved, treat as Bronze but record in logs.
                if not dest:
                    dest = "Bronze"
                ...
                if dest in declined_by_grade:
                    ...
                    # If somehow we get a weird grade, also bucket into Bronze
                    declined_by_grade["Bronze"].append(app)
                    missing_grade.append(app)

            if missing_grade:
                logging.warning(
                    "[MRUM][Slide11] %d declined apps defaulted to Bronze due to missing grade: %s",
                    len(missing_grade),
                    missing_grade[:20],
                )
            ...
            logging.debug(
                "[mrum][Slide11] Declined-by-grade counts: %s",
                {k: len(v) for k, v in declined_by_grade.items()},
            )
            logging.debug(
                "[mrum][Slide11] Final totals_by_grade: %s", totals_by_grade
            )
            ...
            for g in table_grades:
                ...
                logging.info(
                    "[mrum][Slide11] Grade=%s declined=%d total=%d pct=%d%%",
                    g,
                    num_apps,
                    denom,
                    pct,
            ...
            if table:
                ...
                logging.error(
                    "[mrum][Slide11] Could not insert Network Requests Declined table; placeholder missing."
                )

        # ============================
        # Slide 12: Health Rules & Alerting Deep Dive (MRUM)
        ...
        # Helpers (reuse patterns from Slide 11, but with per-metric rules).
        def hra_set_shape_text(slide, shape_name, text):
            if slide is None:
                return False
            for shp in slide.shapes:
                if getattr(shp, "name", "") == shape_name and hasattr(
                    ...
                    return True
            logging.debug(
                "[MRUM] Shape '%s' not found on HRA deep dive slide.", shape_name
            )
            return False

        def hra_first_present_col(df, candidates):
            for c in candidates:
                if c in df.columns:
                    return c
            return None

        def hra_parse_transition_tokens(val):
            s = str(val or "").strip()
            if "→" in s:
                prev, curr = s.split("→", 1)
                return prev.strip(), curr.strip().split("(")[0].strip()
        ...
        def hra_token_to_num(tok):
            try:
                return float(str(tok).strip())
            except Exception:
                return None

        def hra_is_bad_change(val, rule: str) -> bool:
            """
            Returns True if the transition represents a 'downgrade' for the
            given rule:
              - 'lower_better': downgrade when value INCREASES
              - 'higher_better': downgrade when value DECREASES
            Fallback: any 'declined'/'downgraded' wording is treated as bad.
            """
            prev, curr = hra_parse_transition_tokens(val)
            if prev is not None and curr is not None:
                pn = hra_token_to_num(prev)
                cn = hra_token_to_num(curr)
                if pn is not None and cn is not None:
                    if rule == "lower_better":
                        # More violations = worse.
                        return cn > pn
                    elif rule == "higher_better":
                        # Fewer actions/custom rules = worse.
                        return cn < pn
            s = str(val or "").lower()
            return any(k in s for k in ("declined", "downgraded", "decreased", "reduced", "down", "↓"))

        def hra_resolve_metric_col(df, candidates):
            col = hra_first_present_col(df, candidates)
            if col:
                return col
        ...
        # ============================
        # Rectangles 10..12 — count 'bad' changes per metric from df_health_rules.
        # ============================
        if hra_deep_dive_slide:

            logging.debug(
                "[MRUM][Slide12] df_health_rules columns: %s",
                list(df_health_rules.columns),
            )

            # Resolve application column.
            def hra_resolve_app_col(df):
                col = hra_first_present_col(
                    df,
                    ["name", "Name", "applicationName", "Application Name", "Application"],
                )
                if col:
            ...
            app_col_hr_eff = hra_resolve_app_col(df_health_rules)
            logging.debug(
                "[MRUM][Slide12] Rectangles: resolved app_col=%s", app_col_hr_eff
            )

            # Metric columns (common headers + variants).
            col_violations = hra_resolve_metric_col(
                df_health_rules,
                [
                    "numberOfHealthRuleViolations",
                    "NumberOfHealthRuleViolations",
                    "NumberOfHealthRuleViolationsPastDay",
                    "HealthRuleViolations",
                ],
            )
            col_actions_bound = hra_resolve_metric_col(
                df_health_rules,
                [
            ...
            col_custom_rules = hra_resolve_metric_col(
                df_health_rules,
                [
            ...
            logging.debug(
                "[MRUM][Slide12] Rectangles: metric columns -> violations=%s, "
                "actionsBound=%s, customRules=%s",
                col_violations,
                col_actions_bound,
                col_custom_rules,
            )

            def hra_count_metric_bad(df, app_col, metric_col, rule, label):
                if not app_col or not metric_col:
                    logging.warning(
                        "[MRUM][Slide12] Metric '%s' missing column (%s) or app_col (%s).",
                        label,
                        metric_col,
                        app_col,
                    )
                ...
                for _, r in df.iterrows():
                    app = str(r.get(app_col, "") or "").strip()
                    val = r.get(metric_col, "")
                    if hra_is_bad_change(val, rule):
                        apps.append(app)
                logging.info(
                    "[MRUM][Slide12] Rectangles: %s 'bad' changes=%d (apps sample: %s)",
                    label,
                    len(apps),
                    apps[:10],
                )
            ...
            # Apply correct rules:
            # - Violations: LOWER is better → downgrade when value INCREASES.
            # - ActionsBound & CustomRules: HIGHER is better → downgrade when value DECREASES.
            violations_cnt, violations_apps = hra_count_metric_bad(
                df_health_rules,
                app_col_hr_eff,
                col_violations,
                "lower_better",
            ...
            actions_bound_cnt, actions_bound_apps = hra_count_metric_bad(
                df_health_rules,
                app_col_hr_eff,
                col_actions_bound,
                "higher_better",
            ...
            custom_rules_cnt, custom_rules_apps = hra_count_metric_bad(
                df_health_rules,
                app_col_hr_eff,
                col_custom_rules,
                "higher_better",
            ...
            # Write counts into the rectangles.
            hra_set_shape_text(hra_deep_dive_slide, "Rectangle 10", str(violations_cnt))
            hra_set_shape_text(hra_deep_dive_slide, "Rectangle 11", str(actions_bound_cnt))
            hra_set_shape_text(hra_deep_dive_slide, "Rectangle 12", str(custom_rules_cnt))
        ...
        if hra_deep_dive_slide and find_table_placeholder_by_name(
            ...
            logging.debug(
                "[MRUM][Slide12] df_analysis columns: %s", list(df_analysis.columns)
            )

            # Pick the correct Analysis column (BRUM vs MRUM).
            hra_analysis_col = None
            for candidate in ("HealthRulesAndAlertingMRUM", "HealthRulesAndAlertingMRUM"):
                if candidate in df_analysis.columns:
                    hra_analysis_col = candidate
                    break

            if not hra_analysis_col:
                logging.warning(
                    "[MRUM][Slide12] No HealthRulesAndAlerting* column found in Analysis; "
                    "skipping declined HRA table."
                )
            else:
                # Canonical grade order for rank comparisons.
                all_grades = ["platinum", "gold", "silver", "bronze"]
                ...
                def hra_parse_transition_grade(val):
                    """
                    Returns (prev_grade_norm, curr_grade_norm) or (None, None) if not parsable.
                    """
                    s = str(val or "").strip()
                    ...
                    if "→" in s:
                        parts = s.split("→", 1)
                        prev = hra_norm_grade(parts[0])
                        curr = hra_norm_grade(parts[1])
                        return (prev, curr)
                    low = s.lower()
                    import re
                    ...
                    if m:
                        return (None, m.group(1))
                    m = re.search(
                        r"(?:now|is\s+now|became)\s+(platinum|gold|silver|bronze)",
                        low
                    )
                    if m:
                        return (None, m.group(1))
                    g = hra_norm_grade(low)
                    if g:
                        return (None, g)
                    return (None, None)

                # Resolve app and grade columns and prepare row lookup.
                app_col_hr = hra_first_present_col(
                    df_health_rules,
                    ["name", "Name", "applicationName", "Application Name", "Application"],
                ) or next(
                    (
                ...
                grade_col_hr = hra_first_present_col(
                    df_health_rules,
                    [
                        "HealthRulesAndAlertingGrade",
                        "HealthRulesGrade",
                        "MRUMHealthRulesGrade",
                        "Health Rules Grade",
                        "Grade",
                        "grade",
                    ],
                ...
                def resolve_grade_for_app_hr(app):
                    # Prefer explicit grade column in HRA sheet.
                    if grade_col_hr:
                        r = row_for_app_hr(app)
                        if r is not None:
                            g = hra_norm_grade(r.get(grade_col_hr, ""))
                            if g:
                                return g.capitalize()
                    # Scan entire HRA row for grade keywords.
                    r = row_for_app_hr(app)
                    if r is not None:
                        for v in r.values:
                            g = hra_norm_grade(v)
                            if g:
                                return g.capitalize()
                    # Fallback: look in Analysis HRA column (MRUM or BRUM).
                    txt = df_analysis.loc[
                        df_analysis["name"].astype(str).str.strip() == str(app),
                        hra_analysis_col,
                    ]
                    if not txt.empty:
                        _, cg = hra_parse_transition_grade(txt.iloc[0])
                        if cg:
                            return cg.capitalize()
                    return None

                logging.debug(
                    "[MRUM][Slide12] Resolved app_col_hr=%s, grade_col_hr=%s, hra_analysis_col=%s",
                    app_col_hr,
                    grade_col_hr,
                    hra_analysis_col,
                )

                # Build per-grade totals from HRA sheet if possible; else infer from Analysis.
                ...
                if not any(v > 0 for v in totals_by_grade_hr.values()):
                    inferred_totals = {g: 0 for g in table_grades}
                    for _, r in df_analysis.iterrows():
                        _, cg = hra_parse_transition_grade(
                            r.get(hra_analysis_col, "")
                        )
                        disp = cg.capitalize() if cg else None
                    ...
                    if any(inferred_totals.values()):
                        ...
                        logging.warning(
                            "[MRUM][Slide12] Using inferred per-grade totals from Analysis (%s): %s",
                            hra_analysis_col,
                            totals_by_grade_hr,
                        )
                ...
                for _, r in df_analysis.iterrows():
                    app = str(r.get("name", "") or "").strip()
                    val = r.get(hra_analysis_col, "")
                    prev_g, curr_g = hra_parse_transition_grade(val)
                    low = str(val).lower()
                    is_dg = False
                    if "declined" in low or "downgraded" in low:
                        is_dg = True
                ...
                logging.info(
                    "[MRUM][Slide12] Declined apps from Analysis (%s): %d",
                    hra_analysis_col,
                    len(declined_apps_analysis_hr),
                )

                # B) Declines detected from HRA metric columns (using per-metric rules we defined earlier).
                metric_cols_hr = [
                    (col_violations, "lower_better"),
                    (col_actions_bound, "higher_better"),
                    (col_custom_rules, "higher_better"),
                ]
                metric_declined_apps_hr = set()
                if app_col_hr:
                    for col, rule in metric_cols_hr:
                        if not col:
                            continue
                        for _, r in df_health_rules.iterrows():
                            app = str(r.get(app_col_hr, "") or "").strip()
                ...
                logging.info(
                    "[MRUM][Slide12] Declined apps from HRA metrics: %d",
                ...
                logging.info(
                    "[MRUM][Slide12] Total declined apps (union): %d",
                ...
                if missing_grade_hr:
                    logging.warning(
                        "[MRUM][Slide12] %d declined apps have no resolvable grade (not shown in table): %s",
                        len(missing_grade_hr),
                        missing_grade_hr[:20],
                    )

                logging.debug(
                    "[MRUM][Slide12] Declined-by-grade counts: %s",
                    {k: len(v) for k, v in declined_by_grade_hr.items()},
                )
                logging.debug(
                    "[MRUM][Slide12] Final totals_by_grade_hr: %s", totals_by_grade_hr
                )
                ...
                for g in table_grades:
                    ...
                    pct = round((num_apps / denom) * 100) if denom > 0 else 0
                    logging.info(
                        "[MRUM][Slide12] Grade=%s declined=%d total=%d pct=%d%%",
                        g,
                        num_apps,
                        denom,
                        pct,
                ...
                if table:
                    ...
                    logging.error(
                        "[MRUM][Slide12] Could not insert Health Rules & Alerting Declined table; placeholder missing."
                    )
        ...
        if not set_textbox_value(prs, "TextBox 7", number_of_apps, fallback_slide_index=5):
            logging.warning(
                "[mrum] 'TextBox 7' not found; mrum application count not written."
            )
        ...
        log.error("[mrum] Error generating PowerPoint: %s", e, exc_info=True)
